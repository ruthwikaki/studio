...
export default function DashboardPage() {
  ...
  const { data: kpiData, isLoading: kpisLoading, isError: kpisError, error: kpisFetchError } = useAnalyticsDashboard({ refetchInterval: 60000 });

  const handleViewAlerts = async () => {
    toast({ title: "Fetching Alerts...", description: "This may take a moment." });
    router.push('/inventory?lowStockOnly=true');
  };
          ...
          <CardContent className="flex flex-col items-center justify-center text-center min-h-[200px] p-4">
            ...
            (
              <div className="w-full h-full flex flex-col items-center justify-center bg-muted/30 rounded-md p-4">
                ...
                <p className="text-sm text-muted-foreground">Turnover data not available.</p>
              </div>
            )
            }
          </CardContent>
        ...
        <Card className="shadow-lg">
          ...
          <CardContent className="min-h-[200px] p-4">
            <div className="w-full h-full flex flex-col items-center justify-center bg-muted/30 rounded-md p-4">
              ...
              <p className="text-xs text-muted-foreground text-center">Dynamic table with sortable columns.</p>
            </div>
          </CardContent>
        </Card>
      </div>
